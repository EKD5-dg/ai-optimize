...
public static class BigFileScanner
{
    ...
    public static IReadOnlyList<string> GetScanRoots()
    {
        ...
        {
            if (!string.IsNullOrEmpty(path) && Directory.Exists(path) && !roots.Contains(path))
                roots.Add(path);
        }
    ...
    public static Task<List<BigFile>> ScanAsync(long minBytes = DefaultMinBytes, int top = DefaultTop) => Task.Run(() =>
    {
